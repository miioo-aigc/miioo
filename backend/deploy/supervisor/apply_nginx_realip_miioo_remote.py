...
EXTENSION_PATHS = [
    Path("/www/server/panel/vhost/nginx/extension/miiooaib.com/real_ip_whitelist.conf"),
    Path("/www/server/panel/vhost/nginx/extension/miioo.com/real_ip_whitelist.conf"),
]
...
def backup_file(path: Path, timestamp: str) -> None:
    if not path.exists():
        return
    backup_path = path.with_name(f"{path.name}.bak.real_ip_{timestamp}")
...
def main() -> None:
    ...
    run("nginx -t")
    run("nginx -s reload")
    run("curl -k -I -s https://www.miiooai.com/api/providers | head -n 8")
