...
REMOTE_SCRIPT_PATH="/tmp/apply_nginx_apex_media_fallback_miioo_remote.py"
...
main() {
  ...
  echo "上传远端 apex 媒体兜底脚本到 ${REMOTE_TARGET}:${REMOTE_SCRIPT_PATH}"
  run_with_expect scp \
    -o StrictHostKeyChecking=accept-new \
    "${SCRIPT_DIR}/apply_nginx_apex_media_fallback_miioo_remote.py" \
    "${REMOTE_TARGET}:${REMOTE_SCRIPT_PATH}"

  echo "在远端执行 apex 媒体兜底配置修补"
  run_with_expect ssh \
    -o StrictHostKeyChecking=accept-new \
    "${REMOTE_TARGET}" \
