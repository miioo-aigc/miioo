...
REMOTE_SCRIPT_PATH="/tmp/apply_nginx_cdn_realip_chengxvblog_remote.py"
...
main() {
  ...
  echo "上传远端 Nginx CDN real_ip 补丁脚本到 ${REMOTE_TARGET}:${REMOTE_SCRIPT_PATH}"
  run_with_expect scp \
    -o StrictHostKeyChecking=accept-new \
    "${SCRIPT_DIR}/apply_nginx_cdn_realip_chengxvblog_remote.py" \
    "${REMOTE_TARGET}:${REMOTE_SCRIPT_PATH}"

  echo "在远端执行 Nginx CDN real_ip 补丁"
  run_with_expect ssh \
    -o StrictHostKeyChecking=accept-new \
    "${REMOTE_TARGET}" \
