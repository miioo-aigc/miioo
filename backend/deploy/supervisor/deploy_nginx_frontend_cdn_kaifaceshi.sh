...
REMOTE_SCRIPT_PATH="/tmp/apply_nginx_frontend_cdn_kaifaceshi_remote.py"
...
main() {
  ...
  echo "上传远端 kaifaceshi.com 前端 CDN 配置脚本到 ${REMOTE_TARGET}:${REMOTE_SCRIPT_PATH}"
  run_with_expect scp \
    -o StrictHostKeyChecking=accept-new \
    "${SCRIPT_DIR}/apply_nginx_frontend_cdn_kaifaceshi_remote.py" \
    "${REMOTE_TARGET}:${REMOTE_SCRIPT_PATH}"

  echo "在远端执行 kaifaceshi.com 全套前端 CDN 配置"
  run_with_expect ssh \
    -o StrictHostKeyChecking=accept-new \
    "${REMOTE_TARGET}" \
