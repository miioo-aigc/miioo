...
REMOTE_SCRIPT_PATH="/tmp/apply_nginx_frontend_cdn_miioo_remote.py"
...
main() {
  ...
  echo "上传远端 miioo.com 前端 CDN 增强脚本到 ${REMOTE_TARGET}:${REMOTE_SCRIPT_PATH}"
  run_with_expect scp \
    -o StrictHostKeyChecking=accept-new \
    "${SCRIPT_DIR}/apply_nginx_frontend_cdn_miioo_remote.py" \
    "${REMOTE_TARGET}:${REMOTE_SCRIPT_PATH}"

  echo "在远端执行 miioo.com 前端 CDN 配置增强"
  run_with_expect ssh \
    -o StrictHostKeyChecking=accept-new \
    "${REMOTE_TARGET}" \
