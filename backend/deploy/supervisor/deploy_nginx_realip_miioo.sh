...
REMOTE_SCRIPT_PATH="/tmp/apply_nginx_realip_miioo_remote.py"
...
main() {
  ...
  echo "上传远端 real_ip 白名单脚本到 ${REMOTE_TARGET}:${REMOTE_SCRIPT_PATH}"
  run_with_expect scp \
    -o StrictHostKeyChecking=accept-new \
    "${SCRIPT_DIR}/apply_nginx_realip_miioo_remote.py" \
    "${REMOTE_TARGET}:${REMOTE_SCRIPT_PATH}"

  echo "在远端执行生产 real_ip 白名单配置"
  run_with_expect ssh \
    -o StrictHostKeyChecking=accept-new \
    "${REMOTE_TARGET}" \
