...
> 最后更新：2026-07-24（Seedance2.0素材库真人人像 UI）
...
- **2026-07-22 主体参考图与候选图分流修复**：主体编辑弹窗左侧参考图继续使用主体 `reference-images` 上传/绑定接口，仅作为生图输入；右侧候选图列表只接收 `candidate_images` 和生成结果。右侧资产库选择不再绑定主体参考图，本地候选图上传改走通用图片资产上传接口，不再设置 `isReference` 或调用参考图绑定接口。同步复核 StoryboardPage：图片生成面板的 `refImages`、分镜主体参考列 `mainRefs`、候选分镜图 `generatedImages` 和视频参考素材均为独立状态，没有发现参考素材混入候选结果的同类串联。
