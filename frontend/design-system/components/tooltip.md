...
| 圆角 | `rounded-sm`（2px） |
| 内边距 | `px-[8px] py-0.5` |
| 字号 | 默认 `font-size-12`，可按场景配置 |
| 行高 | 默认 `16px`，可按场景配置 |
| 文字色 | `text-text-primary` |
| 描边 | 默认无描边，可按场景配置 `1px` 描边颜色 |
| 出现位置 | 跟随触发元素，默认在上方，支持通过 `placement="bottom"` 显示在下方并水平居中 |
| 多行提示 | 传入包含换行符的纯文本，并设置 `multiline`；未设置时保持单行 |
| 自动换行 | 传入 `maxWidth` 后，在指定最大宽度内按自然断句换行 |
...
- 需要明确控制断句时使用 `multiline` 和换行符；需要随指定宽度自然换行时使用 `maxWidth`
