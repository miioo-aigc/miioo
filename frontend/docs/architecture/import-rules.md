...
当前项目已提供 `npm run check:architecture`，检查文件命名、页面/组件/Hook 规模提醒、基础 UI 反向依赖、规则文档镜像一致性、结构索引占位符，以及 API/反馈/遮罩/业务组件的页面反向依赖。页面 600/900 行、通用 UI 300/500 行、业务区块 500/800 行、Hook 350/500 行分别作为提示线/强提醒线；规模本身不自动阻断，命名、文档镜像不一致、结构索引占位符和非法导入属于阻断级错误。
