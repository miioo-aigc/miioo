...
## 0.1 2026-08-24 仅有 preview_video_url 时视频候选为空

- **现象**：后端分镜响应已有 `preview_video_url`，但创作视频弹窗右侧候选区和分镜候选媒体列表均不展示视频。
- **根因**：`video_url` 可能仍为 `null`，而前端旧适配只用 `video_url` 构造 `storyboardVideo`；同时媒体加载提示判断把 `preview_video_url` 当作“已有媒体”，导致跳过 `media-candidates` 请求，候选映射最终为空。
- **修复**：分镜数据适配优先使用正式 `video_url`，缺失时回退 `preview_video_url`；候选媒体适配遇到 `preview_video_url`、`previewVideoUrl` 或视频地址字段时归一为 `video`；创作面板候选合并时补入当前分镜的主记录视频兜底项。
- **排查经验**：看到列表为空时，不能只检查接口是否返回数据，还要沿着“响应字段 → `url`/`media_type` 归一化 → 跳过请求条件 → 候选合并 → 组件过滤/播放地址”逐层核对。预览地址和正式地址是不同字段，不能假设二者同时存在。
- **验证**：定向 ESLint、`npm run build`、`npm run check:architecture`、`git diff --check` 通过，用户已验证修复通过。

## 0. 2026-07-30 创作结果详情信息修复

- 详情弹窗右侧的参考图不再作为生成参数 URL文本展示，统一转换为缩略图；每张缩略图使用媒体可访问地址，避免用户需要复制链接才能识别参考素材。
- 参考图字段兼容 `reference_images`、`referenceImages`、`reference_image_urls`、`referenceImageUrls`、`ref_images`、`refImages`，并从媒体对象、元数据及 `params`、`gen_params`、`generation_params` 等嵌套容器读取。
- 提示词字段兼容候选媒体、元数据、生成参数和当前镜头的 `image_prompt`/`video_prompt`；本地上传按来源规则继续隐藏提示词与生成参数。
- 普通参数列表过滤参考图字段，避免同一内容重复显示；主体媒体详情弹窗也统一兼容字符串、`url`、`fileUrl` 和 `file_url` 形式的参考图。
- 详情入口由 `MediaCol.jsx` 透传参考图、提示词、画幅比例等媒体元数据，保持候选媒体详情与生成历史数据一致。
...
- 定稿时间轴卡片固定为 `240×135px`，只适合 `16:9` 项目。
- 卡片内图片和视频使用 `object-fit: cover`，素材比例与项目比例不一致时可能被裁切。
...
- 时间轴卡片比例由项目设置决定：
  - `16:9` 项目：`240×135px`。
  - `9:16` 项目：`135×240px`。
- 单个候选媒体的原始比例不改变卡片尺寸。即使 `16:9` 项目中的候选是 `9:16` 或 `1:1`，仍保持横屏卡片；`9:16` 项目中的横屏候选也仍保持竖屏卡片。
- 图片和视频统一使用 `object-fit: contain`，保证内容完整展示；不一致比例产生的上下或左右留白由卡片深色背景填充。
- 竖屏项目的时间轴面板最小高度调整为 `272px`，横屏项目继续使用 `167px`，避免竖屏卡片被裁切。

### 7.3 组件边界

- `StoryboardPage` 只传入项目画幅 `projectRatio`。
- `StoryboardFinalizedTimeline` 负责把项目画幅转换为卡片尺寸。
- `StoryboardFinalizedCard` 负责按传入尺寸渲染图片、视频、空卡片和悬停交互。
- 候选列表、统一详情弹窗和下载链路不受本次画幅适配影响。
