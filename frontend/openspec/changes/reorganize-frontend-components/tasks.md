...
- [ ] 5.3 完成样板页构建、Lint、架构检查和关键交互回归，记录迁移前后差异
  - [x] 5.3.a 构建、架构检查和首页/项目页有限浏览器回归通过
  - [x] 5.3.b 完整 lint 已通过；样板页仍需按业务范围补充更完整的关键流程回归（2026-07-16）
...
- [x] 6.3 完成 `AssetsPage` 静态迁移：资产筛选、资产网格、详情/批量动作和弹窗组合已按边界拆分；真实业务回归另列为未完成项
  ...
  - [ ] 6.3.e.4 完成 AssetsPage 卡片/详情组合收尾及登录态关键流程回归（音频播放/收藏延期，待具备可见音频数据）
    ...
    - [ ] 6.3.e.4.c 具备足够测试数据后验证分页真实触发、创作配音卡片播放/收藏和各详情类型逐类流程（当前无可见音频卡片，音频部分延期）
    - [ ] 6.3.e.4.d 在安全测试条件具备并经确认后验证删除、下载、上传等有外部副作用的流程
...
- [x] 6.4 完成 `CreationPage` 静态迁移：创作工具栏、输入区、生成区、历史区和生成动作的可安全展示/适配边界已拆分；登录态运行时验收另列为未完成项
...
- [x] 6.4.j 完成创作页运行时验收（2026-07-17；按用户确认完成，未要求重复执行）
  - 静态安全修复已完成；以下运行时分支仍待具备安全测试条件后逐项验证，不得因静态检查通过提前标记完成。
...
- [x] 6.5 完成 `StoryboardPage` 静态迁移：分镜表格、镜头行、批量工具栏、媒体列和生成流程的可安全展示/适配边界已拆分；登录态副作用回归另列为未完成项
...
- [x] 6.5.g 完成 StoryboardPage 登录态非破坏性关键流程回归（2026-07-17；真实生成、上传、下载、删除、定稿和配音仍单独未验证）
  ...
  - [x] 6.5.g.4 在已有测试媒体数据下完成参考素材/上传槽位展示核对、图片详情和视频查看器打开/关闭验证；未执行文件选择、真实上传、下载、删除、定稿或生成等外部副作用。参考素材编辑提交和逐类外部副作用流程仍待后续安全条件具备后验证
...
> 进度说明：6.1–6.5 的安全静态拆分范围已完成，6.6 已同步 Home、StoryboardPage、SubjectPage 和 CreationPage 的最新组件边界与结构索引；页面入口仍保留认证、API、任务轮询、缓存、Toast、Store 写回和上传/生成/删除/下载/定稿等副作用。当前下一步不再为了压缩行数强拆页面副作用，而是在安全登录态、可回收测试项目和测试素材下，按 CreationPage → AssetsPage → StoryboardPage 顺序完成任务恢复、失败恢复、编辑回填、分页及外部副作用流程回归；条件不足的项目继续明确记录为未验证，完成后再做全量业务回归和 OpenSpec 收尾。

## 7. 收尾与文档同步

- [x] 7.1 使用文档同步流程检查 AGENTS.md、CLAUDE.md、PROJECT.md、README.md 和 docs/ 的路径、命令及进度是否真实（2026-07-15）
- [x] 7.2 清理已废弃的重复基础按钮实现和无效迁移记录：创作结果卡片的重复 `CardActionBtn` 已统一为 `CreationCardActionButton`；无引用历史备份 `src/components/LoginModal.jsx.bak` 保留，删除与否待用户确认（2026-07-16）
...
- [ ] 7.4 复核所有迁移页面的关键用户流程，确认无接口、视觉和行为回归
  ...
  - [ ] 7.4.b 完成需登录/后端接口支持的业务流程回归；真实生成、上传、下载、删除、收藏、定稿、配音和任务恢复仍待明确授权与测试对象
- [x] 7.5 完成 OpenSpec 变更归档前审查；未完成的登录态、测试数据和外部副作用任务已单独保留，待运行时回归完成后归档（2026-07-16）

> 2026-07-16 文档整理结果：规则文档已从“建议集合”重写为项目总规则、专项规则、架构规则和可执行检查四层；OpenSpec 已完成归档前审查，但未因运行时任务未完成而提前归档；页面 300 行、通用 UI 250 行、业务区块 400 行、Hook 300 行、单函数 50 行均定义为警告线，不作为当前无条件阻断。最终静态门禁已全部通过；真实业务回归和页面规模告警治理仍未完成，不能据此归档 OpenSpec 变更。
