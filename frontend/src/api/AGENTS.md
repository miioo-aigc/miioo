...
1. 先阅读 `src/api/api文档.json`，确认路径、方法、请求体、查询参数、鉴权要求和响应字段。
...
当 `api文档.json` 发生变化时，必须通读相关 `src/api/*.js` 和所有调用方，完成全量差异分析：

- 路径、HTTP 方法和鉴权方式；
- 路径参数、查询参数和请求体字段；
- 前端缺少的接口和前端多出的 legacy alias；
- 响应字段、错误结构和分页/任务状态结构；
- 现有页面调用是否需要补充 `projectId` 等上下文。

差异结论更新到 API 审计文档或对应迁移记录；不能只修第一处报错后停止。
