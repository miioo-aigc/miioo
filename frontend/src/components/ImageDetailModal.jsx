...
export default function ImageDetailModal({ card, onClose, onDelete, favorited, onToggleFavorite, onDownload }) {
            ...
            <div style={{ display: 'flex', height: '540px' }}>
              ...
              <div style={{ width: '280px', flexShrink: 0, backgroundColor: '#161616', borderLeft: '1px solid #FFFFFF0F', display: 'flex', flexDirection: 'column', height: '540px' }}>
                ...
                <div style={{ display: 'flex', gap: '8px', padding: '16px 20px 20px', flexShrink: 0, borderTop: '1px solid #FFFFFF0A' }}>
                  ...
                  <ModalActionBtn
                    label="下载"
                    onClick={() => (onDownload ? onDownload(card) : downloadMediaFile(
                      { ...card, assetId: card.assetId || card.asset_id || card.id || null },
                      {
                        prompt: card.prompt,
                        assetName: card.name,
                        mediaType: 'image',
