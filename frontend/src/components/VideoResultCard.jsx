...
function VideoResultCard({ status, videoUrl, prompt, model, ratio, resolution, duration, refImages, createdAt, onReEdit, onUseAsFirstFrame, onDelete, onCardClick, batchMode = false, isSelected = false, onToggleSelect, favorited = false, onToggleFavorite }) {
  ...
  useEffect(() => {
    ...
    if (hovered && isDone) { videoRef.current.play().catch(() => {}); }
    ...
    <>
      <div
        style={{
          ...
          position: 'relative', cursor: isDone ? 'pointer' : 'default',
          outline: isSelected ? '2px solid #2DC3E1' : 'none', outlineOffset: '-2px',
        }}
        onMouseEnter={() => setHovered(true)}
        onMouseLeave={() => setHovered(false)}
        onClick={() => { if (batchMode && isDone) { onToggleSelect?.(); return; } if (!batchMode && isDone) { onCardClick?.(); } }}
      >
        {status === 'loading' ? (
          <div className="creation-shimmer" style={{ width: '100%', height: '100%' }} />
        ) : isDone ? (
          <video ref={videoRef} src={videoUrl} loop muted playsInline style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
        ) : (
        ...
        {hovered && isDone && !batchMode && (
          <>
            ...
            <div style={{ position: 'absolute', bottom: '8px', right: '8px', display: 'flex', gap: '4px' }} onClick={(e) => e.stopPropagation()}>
              ...
              <CardActionBtn tooltip="尾帧用作首帧参考" onClick={() => onUseAsFirstFrame?.()}
                icon={<svg width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" style={{ width: '16px', height: '16px', overflow: 'visible' }}><path d="M9.446 1.733C9.888 1.733 10.246 2.092 10.246 2.533V21.855C10.246 22.297 9.888 22.655 9.447 22.655C9.005 22.655 8.646 22.297 8.646 21.855V2.533C8.646 2.092 9.005 1.733 9.447 1.733H9.446Z" fill="#FFFFFF" /><path d="M9.194 3.483V5.083H4.706C4.411 5.083 4.172 5.322 4.172 5.617V18.946C4.172 19.241 4.411 19.479 4.706 19.479H9.194V21.079H4.706C3.527 21.079 2.572 20.124 2.572 18.946V5.617C2.572 4.438 3.527 3.483 4.706 3.483H9.194Z" fill="#FFFFFF" /><path d="M14.957 3.483V5.083H19.446C19.74 5.083 19.979 5.322 19.979 5.617V18.946C19.979 19.241 19.74 19.479 19.446 19.479H14.957V21.079H19.446C20.624 21.079 21.579 20.124 21.579 18.946V5.617C21.579 4.438 20.624 3.483 19.446 3.483H14.957Z" fill="#FFFFFF66" /><path d="M20.339 8.787H14.707V7.187H20.339V8.787ZM20.339 17.402H14.707V15.802H20.339V17.402Z" fill="#FFFFFF66" /><path d="M 3 9.1 L 8.632 9.1 L 8.632 7.5 L 3 7.5 L 3 9.1 Z M 3 17.715 L 8.632 17.715 L 8.632 16.115 L 3 16.115 L 3 17.715 Z" fill="#FFFFFF" /><path d="M 14.3 22.422 C 14.742 22.422 15.1 22.064 15.1 21.622 L 15.1 2.3 C 15.1 1.859 14.742 1.5 14.3 1.5 C 13.858 1.5 13.5 1.859 13.5 2.3 L 13.5 21.622 C 13.5 22.064 13.858 22.422 14.3 22.422 Z" fill="#FFFFFF66" /></svg>} />
              <CardActionBtn tooltip="下载" onClick={() => { if (!videoUrl) return; const a = document.createElement('a'); a.href = videoUrl; a.download = 'creation.mp4'; document.body.appendChild(a); a.click(); document.body.removeChild(a); }}
      ...
      {confirmDelete && (
        <ConfirmDialog title="确认删除" description="删除后无法恢复，确定要删除此视频吗？" confirmText="删除"
