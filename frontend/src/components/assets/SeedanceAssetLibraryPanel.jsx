 ...
 *   2026-09-02  素材组新增、重命名、删除的 400 错误透传 detail，并通过确认型 Notice 展示
 */
...
function getBadRequestDetail(error) {
  if (error?.status !== 400) return '';
  return error.detail || error.message || '请求失败（400）';
}
...
export default function SeedanceAssetLibraryPanel({ initialTab = 'real' }) {
  ...
  const handleRename = async () => {
    ...
    try {
      ...
      const detail = getBadRequestDetail(error);
      if (detail) {
        setRenameTarget(null);
        setRenameValue('');
        setNotice({ title: '重命名失败', message: detail });
      } else {
        showToast('重命名素材组失败，请重试');
      }
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if (detail) {
        setDeleteTarget(null);
        setNotice({ title: '删除失败', message: detail });
      } else {
  ...
  const handleCreateVirtualGroup = async () => {
    ...
    try {
      ...
      const detail = getBadRequestDetail(error);
      if (detail) {
        setNotice({ title: '新建失败', message: detail });
      } else {
