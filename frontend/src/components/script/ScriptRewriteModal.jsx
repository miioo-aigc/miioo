...
export default function ScriptRewriteModal({ open, submitting = false, error = '', onSubmit, onClose }) {
  ...
  const handleSubmit = async () => {
    await onSubmit?.({ instruction: instruction.trim() });
  };
  ...
  return createPortal(
    ...
    >
      <div role="dialog" aria-modal="true" aria-labelledby="script-rewrite-title" style={{ display: 'flex', width: '400px', maxWidth: 'calc(100vw - 32px)', flexDirection: 'column', overflow: 'hidden', borderRadius: '16px', background: '#161616', boxShadow: '0 8px 32px rgba(0,0,0,0.6)', fontFamily: FONT, color: '#FFFFFF' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', padding: '16px 24px' }}>
          ...
          <Button type="button" variant="primary" loading={submitting} onClick={handleSubmit}>开始重写</Button>
        </div>
      </div>
    </div>,
    document.body,
