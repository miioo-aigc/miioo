/**
 * @file ImageResultCard.jsx
 * @structure-index
 *
 * ─── 组件职责 ───────────────────────────────────────────────
 *   图片结果展示       展示图片、加载态和定稿状态
 *   结果卡片交互       透传定稿、查看和下载回调
 *
 * ─── 依赖边界 ───────────────────────────────────────────────
 *   只接收图片 URL、定稿状态和显式回调；不写入结果列表、不调用 API，
 *   不直接创建下载链接或维护页面级状态。
 */
...
export default function ImageResultCard({ settled, imageUrl, onView, onSettledChange, onDownload }) {
