      ...
      {detailOpen && category !== 'storyboard_img' && category !== 'storyboard_video' && images.length > 0 && (
        <SubjectAssetDetailModal
          ...
          onDownload={(imageId, fileUrl) => {
            ...
            if (!img) return;
            downloadFile({
              responseFetcher: () => apiDownloadAsset(img.id, { prefer_origin: true, rawResponse: true }),
              url: img.downloadUrl || img.fileUrl || fileUrl || img.url || '',
              prompt: img.prompt || '',
              assetName: img.name || name,
              mediaType: img.videoUrl ? 'video' : 'image',
              contextLabel: '项目资产',
            }).catch(console.error);
          }}
...
function ProjectAssetsPanel() {
  ...
  function handleDownloadProject(project) {
    downloadFile({
      responseFetcher: () => apiDownloadProjectAssets(project.id, { rawResponse: true }),
  ...
  async function downloadAsset(asset) {
    ...
    try {
      await downloadFile({
        responseFetcher: () => apiDownloadAsset(asset.id, { prefer_origin: true, rawResponse: true }),
        url: asset.downloadUrl || asset.fileUrl || asset.videoUrl || asset.url || '',
        prompt: asset.prompt || '',
        assetName: asset.name,
        mediaType: asset.videoUrl ? 'video' : asset.audioUrl ? 'audio' : 'image',
        contextLabel: '项目资产',
      });
    } catch (err) {
  ...
  function getSelectedDownloadItems() {
    ...
    selectedIds.forEach((cardId) => {
      ...
      if (Array.isArray(card.images) && card.images.length > 0) {
        card.images.forEach((image, index) => {
          items.push({
            ...
            prompt: image.prompt || card.prompt || '',
            mediaType: image.videoUrl ? 'video' : 'image',
            downloadUrl: image.downloadUrl || image.fileUrl || image.url || '',
          });
        });
        return;
      }
      ...
      items.push({
        ...
        prompt: card.prompt || '',
        mediaType: card.videoUrl ? 'video' : card.audioUrl ? 'audio' : 'image',
        downloadUrl: card.downloadUrl || card.fileUrl || card.videoUrl || card.url || '',
      });
    });
