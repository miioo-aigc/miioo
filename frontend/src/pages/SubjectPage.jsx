...
function ImageViewModal({ open, imageUrl, imageId, projectId, subjectId, onClose }) {
  ...
  async function handleDownload() {
    ...
    try {
      await downloadFile({
        responseFetcher: () => apiDownloadSubjectImage(projectId, subjectId, imageId, { rawResponse: true }),
        url: imageUrl,
        prompt: '',
        mediaType: 'image',
        contextLabel: '主体图',
...
function EditSubjectPanel({ projectId, char, tabLabel = '角色', projectRatio, onClose, onCommit, onCoverChange, refreshToken, setBatchLoadingSubjects }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        merged = list.map((m) => {
          ...
          return {
            ...
            supportsOutpainting: caps.supports_outpainting === true,
          };
        });
      ...
      // 角色已有旧模型但当前管理员未开放时，自动回退到当前仍可用的默认模型。
      if (merged.length > 0) {
        const preferredModelId = char?.model || char?.default_image_model || '';
        const preferredStillAvailable = preferredModelId && merged.some((item) => item.value === preferredModelId);
  ...
  function getFallbackModels() {
    return [
      { value: 'doubao-seedream-5.0-lite', label: 'Doubao-Seed-5.0-Lite', resolutions: ['2K','3K','4K'], resolutionSizeMap: {}, ratios: ['1:1','16:9','9:16','4:3','3:4'], maxRefImages: 3, supportsSubjectCompletion: false, supportsOutpainting: false },
      { value: 'doubao-seedream-4.5', label: 'Doubao-Seed-4.5', resolutions: ['2K','4K'], resolutionSizeMap: {}, ratios: ['1:1','16:9','9:16','4:3','3:4'], maxRefImages: 3, supportsSubjectCompletion: false, supportsOutpainting: false },
      { value: 'doubao-seedream-4.0', label: 'Doubao-Seed-4.0', resolutions: ['1K','2K','4K'], resolutionSizeMap: {}, ratios: ['1:1','16:9','9:16','4:3','3:4'], maxRefImages: 3, supportsSubjectCompletion: false, supportsOutpainting: false },
    ];
  }
  ...
  useEffect(() => {
    ...
    if (imageModels.some((item) => item.value === selectedModel)) return;
    const def = imageModels.find((item) => item.is_default) || imageModels[0];
    if (def?.value && def.value !== selectedModel) {
      setSelectedModel(def.value);
    }
  ...
  const generationModeOptions = useMemo(() => {
    const options = [
      { key: 'main', label: '单视图' },
      { key: 'three_view', label: '多视图' },
    ];
    if (tabLabel === '角色' && currentModel.supportsSubjectCompletion) {
      options.push({ key: 'subject_completion', label: '主体补全' });
    }
    return options;
  }, [currentModel.supportsSubjectCompletion, tabLabel]);

  useEffect(() => {
    if (!generationModeOptions.some((option) => option.key === genMode)) {
      setGenMode(generationModeOptions[0]?.key || 'main');
    }
  }, [generationModeOptions, genMode]);
          ...
          {tabLabel === '角色' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              <div style={{ display: 'flex', gap: '24px', alignItems: 'center' }}>
                {generationModeOptions.map(({ key, label }) => {
          ...
          {generatedImages.map((img, i) => (
            <ImageItem
              ...
              onDownload={async () => {
                try {
                  await downloadFile({
                    responseFetcher: () => apiDownloadSubjectImage(projectId, char.id, img.id, { rawResponse: true }),
                    url: img.url,
                    prompt: img.prompt || char.prompt || '',
                    mediaType: 'image',
                    contextLabel: '主体图',
...
export default function SubjectPage({ projectId, projectName = '两只老虎的奇遇', onBack, onUnlockStep, onStartStoryboard, onExtractSubjects, extractError = null, isStoryboardGenerated = false, initialTab = 'char', projectRatio, chars: externalChars, onCharsChange, scenes: externalScenes, onScenesChange, props: externalProps, onPropsChange, onLoadMoreChars, onLoadMoreScenes, onLoadMoreProps, hasMoreChars = false, hasMoreScenes = false, hasMoreProps = false }) {
  ...
  const handleDownloadSubjectImage = async (subjectId) => {
    try {
      ...
      await downloadFile({
        responseFetcher: () => apiDownloadSubjectImage(projectId, subjectId, targetImg.id, { rawResponse: true }),
        url: targetImg.url || targetImg.imageUrl || '',
        prompt: targetImg.prompt || '',
        mediaType: 'image',
        contextLabel: '主体图',
        subjectName: chars.find((item) => item.id === subjectId)?.name
          || scenes.find((item) => item.id === subjectId)?.name
          || props.find((item) => item.id === subjectId)?.name
          || subjectId,
      });
    } catch (err) {
      console.error('[SubjectPage] 下载图片失败:', err);
